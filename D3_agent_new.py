#!/usr/bin/env python3
"""
D3_agent_new.py - UAV3 Agent with Two-Phase Task Selection
...
class DroneAgent:
    ...
    def calculate_bid_score(self, task, current_lat=None, current_lon=None):
        ...
        # UAV3 specialization: General purpose - no specific bonus
...
async def handle_task_selection(task, drone, agent, bid_tx):
    ...
    print(f"[{AGENT_ID}] 📤 Sent P2P bid to peers: {my_score}")
...
async def main():
    print("[DEBUG] Entered main() for UAV3")
    print(f"[{AGENT_ID}] 🚁 Starting UAV3 Agent with Two-Phase Task Selection")
