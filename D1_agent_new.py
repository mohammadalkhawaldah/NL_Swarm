...
- MAVSDK: gRPC port 50040 (UAV1, no cross-talk)
"""
...
MY_BID_PORT = UAV1_BID_PORT     # This agent (UAV1) listens here
PEER_BID_PORT = UAV2_BID_PORT   # Send bids to peer (UAV2) here
...
class DroneAgent:
    ...
    async def check_resources(self, drone, task):
        ...
        if current_battery_percent is None:
            ...
            current_battery_percent = 85.0  # UAV1 default
    ...
    def calculate_bid_score(self, task, current_lat=None, current_lon=None):
        ...
        # UAV1 specialization: Better for search/SAR missions
        if task_type in ['search', 'sar', 'search_and_rescue']:
...
async def listen_peers(rx):
    """Listen for peer heartbeats (same as working system)"""
    loop = asyncio.get_running_loop()
    last_peer_print = 0.0
    global last_peer_log_time, last_seen, peer_data
    while True:
        try:
...
async def listen_tasks(rx, tx, drone, agent, bid_rx, bid_tx):
    """Listen for operator tasks and handle bidding"""
    loop = asyncio.get_running_loop()
    while True:
        try:
            ...
            if "task_id" in task and "type" in task:
                print(f"\n[{AGENT_ID}] 📋 Received task: {task['task_id']}")
...
async def listen_bids(bid_rx):
    """Listen for peer bids on point-to-point socket"""
    loop = asyncio.get_running_loop()
    while True:
        try:
            ...
            if "drone_id" in bid_msg and "score" in bid_msg:
                ...
                if task_id:
                    with score_lock:
                        ...
                        if not any(b["drone_id"] == bid_msg["drone_id"] for b in task_scores[task_id]):
                            task_scores[task_id].append(bid_msg)
                    msg_type = bid_msg.get("type", "bid")
                    if msg_type == "win_announcement":
                        print(f"[{AGENT_ID}] 🏆 Received win announcement from {bid_msg['drone_id']} for task {task_id}")
                    else:
                        print(f"[{AGENT_ID}] \U0001F4CA Received P2P bid from {bid_msg['drone_id']}: {bid_msg['score']} for task {task_id}")
        except BlockingIOError:
            await asyncio.sleep(0.05)
        except Exception as e:
...
async def handle_task_selection(task, drone, agent, bid_tx):
    ...
    print(f"[{AGENT_ID}] My bid score: {my_score}")
    score_msg = {
        "drone_id": AGENT_ID,
        "task_id": task_id,
        "score": my_score
    ...
    while time.time() < timeout_time:
            ...
            if task_id in task_scores:
                for score_data in task_scores[task_id]:
                    if score_data["drone_id"] != AGENT_ID and score_data not in peer_scores:
                        peer_scores.append(score_data)
                del task_scores[task_id]
        # Print bids immediately when received
        now = time.time()
        if peer_scores and now - last_bid_print > 0.5:
            ...
            last_bid_print = now
            break  # Exit immediately when peer score received
            
        # 🚀 FAST BIDDING: Check every 100ms instead of 1000ms
        await asyncio.sleep(0.1)
    
    # 🚀 FAST BIDDING: Final quick check for late arrivals
    await asyncio.sleep(0.1)  # Very short final delay
    with score_lock:
        if task_id in task_scores:
            for score_data in task_scores[task_id]:
                if score_data["drone_id"] != AGENT_ID and score_data not in peer_scores:
...
async def main():
    """Main agent loop"""
    print(f"[{AGENT_ID}] 🚁 Starting UAV1 Agent with Two-Phase Task Selection")
